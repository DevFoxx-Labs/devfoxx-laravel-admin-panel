import PackageAdminLayout from '../../../../../Layouts/PackageAdminLayout';
...
export default function GalleryIndex({ galleryItems, filters, stats }) {
    ...
    const handleSubmit = () => {
        ...
        if (editing) {
            router.post(route('admin-panel.gallery.update', editing.id), { ...data, _method: 'put' }, options);
            return;
        }

        router.post(route('admin-panel.gallery.store'), data, options);
    };

    const applyFilters = () => {
        router.get(route('admin-panel.gallery.index'), {
    ...
    return (
        <PackageAdminLayout title="Gallery Management" breadcrumb="Package / Gallery">
                ...
                <>
                            ...
                            {galleryItems.data.map((item) => (
                                <Col xs={24} sm={12} lg={8} xl={6} key={item.id}>
                                    <GalleryCard
                                        item={item}
                                        onEdit={openEdit}
                                        onDelete={(record) => router.delete(route('admin-panel.gallery.destroy', record.id))}
                                        onRestore={(record) => router.patch(route('admin-panel.gallery.restore', record.id))}
                                        onForceDelete={(record) => router.delete(route('admin-panel.gallery.force-delete', record.id))}
                                        onToggleFeatured={(record) => router.patch(route('admin-panel.gallery.toggle-featured', record.id))}
                                        onToggleActive={(record) => router.patch(route('admin-panel.gallery.toggle-active', record.id))}
                                    />
                                </Col>
                            ))}
                        </Row>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
                        <Pagination
                            ...
                            onChange={(page) => router.get(route('admin-panel.gallery.index'), {
                                page,
                                search: search || undefined,
                                status: status || undefined,
                                media_type: mediaTypeFilter || undefined,
