...
class SmsChannel
{
    ...
    public function send(NotificationCampaign $campaign, User $user, array $content): array
    {
        ...
        if ($driver === 'log') {
            Log::info('Simulated SMS notification delivery.', [
        ...
        if ($driver !== 'twilio') {
            return [
                ...
                'error_message' => 'Unsupported SMS driver configured.',
        ...
        if (! $sid || ! $token || ! $from) {
            return [
                ...
                'error_message' => 'Twilio SMS credentials are incomplete.',
                'payload' => null,
            ];
        }
        ...
        if ($response->failed()) {
            return [
                ...
                'error_message' => $response->json('message') ?: 'Twilio SMS request failed.',
